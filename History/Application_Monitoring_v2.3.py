...
key_buffer = {}
MERGE_INTERVAL = 5  # 5 seconds merge interval
        ...
        try:
            ...
            if is_active or not TARGET_APPS:
                if app_name != last_app_name or window_title != last_window_title:
                    if last_app_name:
                        save_queue.put(('save', last_app_name, last_window_title, key_buffer.copy()))
                        key_buffer.clear()
...
def save_thread():
    while True:
        try:
            ...
            if action == 'save':
                save_compressed_file(app_name, window_title, [(k, v['count'], v['first_time']) for k, v in data.items()])
...
def save_data_on_exit():
    save_queue.put(('save', current_app_name, current_window_title, key_buffer.copy()))
